...
from ps5_helpers import (
    ...
    generate_line_of_complete_subgraphs,
...
def run_tests(tests):
        ...
        for i, result in enumerate(test_results):
            ...
            if timedout:
                ...
                print(
                    ...
                    + result["label"]
                    + " (n = {}, m = {})".format(
                        input.N, sum([len(v_lst) for v_lst in input.edges]) // 2
                    )
                    + ": ",
                    "Passed" if con else "Failed",
                    end="",
                )
                if not con and (
                    "show_expectation" not in result or result["show_expectation"]
                ):
                    print(
                        " - Expected " + str(expected) + " But Got " + str(res), end=""
                    )
                    if result["hint_text"]:
                        print(
                            "\n\t"
    ...
    if number_passed != total_tests:
        ...
        print(
            "\t Does your algorithm work with a small number of nodes (specifically < 3)"
        )
        print(
            "\t Do you validate the subset that you selected? (i.e. check that S is an independent set)"
        )
        print("")
    print(
        color.BOLD
        + "Tests Passed {}{}/{}".format(col, number_passed, total_tests)
        + color.END
    )
...
testcases = {
    "2-colorable": [
        ...
        .add_edge(1, 5)
        .add_edge(2, 6)
        .add_edge(3, 7),  # sqaure with line out of each node
        lambda: Graph(8)
        .add_edge(0, 1)
        .add_edge(1, 2)
        .add_edge(2, 3)
        ...
        .clone_and_merge(
            ...
            None,
            None,
        ),  # sqaure with line out of each node
        lambda: Graph(8)
        .add_edge(0, 1)
        .add_edge(1, 2)
        .add_edge(2, 3)
    ...
    "3-colorable": [
        ...
        .add_edge(0, 1)
        .add_edge(1, 2)
        .add_edge(2, 0),  # 3 node triangle
        lambda: Graph(3)
        .add_edge(1, 2)
        .add_edge(2, 0)
        .add_edge(0, 1),  # 3 node triangle
        lambda: Graph(3)
        .add_edge(2, 0)
        .add_edge(0, 1)
        .add_edge(1, 2),  # 3 node triangle
        lambda: Graph(6)
        .add_edge(2, 0)
        .add_edge(0, 1)
        .add_edge(1, 2)
        .add_edge(3, 4)
        .add_edge(4, 5)
        .add_edge(5, 3),  # 3 node triangle x2 disconnected
        lambda: Graph(6)
        .add_edge(0, 1)
        .add_edge(1, 2)
        .add_edge(2, 0)
        ...
        .add_edge(0, 3)
        .add_edge(1, 4)
        .add_edge(1, 5),  # 3 node triangle with line out of each node
        ...
        .add_edge(1, 7)
        .add_edge(1, 8),
        lambda: generate_complete_graph(Graph, 3)
        .clone_and_merge(generate_complete_graph(Graph, 3), 2, 0)
        .clone_and_merge(generate_complete_graph(Graph, 3), 5, 0)
        .clone_and_merge(generate_complete_graph(Graph, 3), 8, 0),
    ...
    "k>3-colorable": [
        ...
        lambda: generate_complete_graph(Graph, 3).clone_and_merge(
            generate_complete_graph(Graph, 4), None, None
        ),
        lambda: generate_complete_graph(Graph, 4).clone_and_merge(
            generate_complete_graph(Graph, 3), None, None
        ),
        lambda: generate_complete_graph(Graph, 7).clone_and_merge(
            generate_complete_graph(Graph, 3), None, None
        ),
        lambda: generate_complete_graph(Graph, 2).clone_and_merge(
            generate_complete_graph(Graph, 4), None, None
        ),
        lambda: generate_complete_graph(Graph, 4).clone_and_merge(
            generate_complete_graph(Graph, 2), None, None
        ),
        lambda: generate_complete_graph(Graph, 7).clone_and_merge(
            generate_complete_graph(Graph, 2), None, None
        ),
...
def test_2_coloring():
    ...
    if INCLUDE_TESTS_FOR_STAFF_CODE:
        ...
        tests["Exhaustive 2-Coloring (Staff Provided)"] += [
            generate_test(
                "Should work on 2-colorable graphs",
                g,
                lambda g: validate_graph_coloring(g, exhaustive_search_coloring(g, 2)),
                lambda g: True,
            )
            for g in testcases["2-colorable"]
        ]
        ...
        tests["BFS 2-Coloring (Staff Provided)"] += [
            generate_test(
                "Should work on 2-colorable graphs",
                g,
                lambda g: validate_graph_coloring(g, bfs_2_coloring(g)),
                lambda g: True,
            )
            for g in testcases["2-colorable"]
        ]
    ...
    if INCLUDE_TESTS_FOR_STAFF_CODE:
        ...
        tests["Exhaustive 3-Coloring (Staff Provided)"] += [
            generate_test(
                "Should work on 2-colorable graphs",
                g,
                lambda g: validate_graph_coloring(g, exhaustive_search_coloring(g, 3)),
                lambda g: True,
            )
            for g in testcases["2-colorable"]
        ]
        tests["Exhaustive 3-Coloring (Staff Provided)"] += [
            generate_test(
                "Should work on 2-or-3-colorable graphs",
                g,
                lambda g: validate_graph_coloring(g, exhaustive_search_coloring(g, 3)),
                lambda g: True,
            )
            for g in testcases["2-or-3-colorable"]
        ]
        tests["Exhaustive 3-Coloring (Staff Provided)"] += [
            generate_test(
                "Should work on 3-colorable graphs",
                g,
                lambda g: validate_graph_coloring(g, exhaustive_search_coloring(g, 3)),
                lambda g: True,
            )
            for g in testcases["3-colorable"]
        ]
        tests["Exhaustive 3-Coloring (Staff Provided)"] += [
            generate_test(
                "Should not work on k>3-colorable graphs",
                g,
                lambda g: validate_graph_coloring(g, exhaustive_search_coloring(g, 3)),
                lambda g: False,
            )
            for g in testcases["k>3-colorable"]
        ]
        ...
        tests["ISET 3-Coloring (Staff Provided)"] += [
            generate_test(
                "Should work on 2-colorable graphs",
                g,
                lambda g: validate_graph_coloring(g, iset_bfs_3_coloring(g)),
                lambda g: True,
            )
            for g in testcases["2-colorable"]
        ]
