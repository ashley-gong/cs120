from ps1 import countSort, mergeSort, radixSort
import random
import time
import itertools
import sys
...
a plot of the transition points (to be submitted in your problem set for 3d). 
...
n_U_set = list(
    itertools.product(
        [int(math.pow(2, x)) for x in range(1, 17)],
        [int(math.pow(2, x)) for x in range(0, 21, 1)],
    )
)
...
def compute_average_time(algo, A, u, n, N=3):
...
def plot_graph():
    ...
    # Counting Sort - blue, Merge Sort - red, Radix Sort - green
    colors = []
    ...
    for combo in n_U_set:
        ...
        countSort = ("blue", compute_average_time("countSort", arr, U, n, N))
        radixSort = ("green", compute_average_time("radixSort", arr, U, n, N))

        min_val = sys.maxsize
        min_color = ""

        for algorithm in [mergeSort, countSort, radixSort]:
            color, t = algorithm
            if t < min_val:
                min_val = t
                min_color = color
